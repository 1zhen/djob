...
Shard.prototype.clearRunningWatcher = function () {
        ...
        try {
            ...
            if (this.status === 'running') {
                ...
                this.$runningWatcherTimer = setInterval(() => {
                    this.$etcd.get(path.join(this.$jobHeartbeatRootKey, status.master)).catch((err) => {
                        if (err.errorCode === 100) {
                            ...
                            this.setStatus('ready', {
                                ...
                                if (err.errorCode === 101) {
                                    ...
                                    this.$logger.error('Some other error occured when reset status.', err);
                                }
                            })
                        }
                    });
